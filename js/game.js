...
async function loadGames() {
  ...
  const playerCaps = walletContents.nfts
  ...
  games = sharedObjects.map((sharedObject) => {
    const {
      details: {
        data: { fields },
      },
...
async function listGames() {
  ...
  const sortedGames = games.sort((a, b) =>
    a.winner && b.winner ? 0 : a.winner ? 1 : -1
  );
  for (const game of sortedGames) {
    ...
    const turn =
      game.current_player === address ? "Your Turn" : "Opponent's Turn";
    ...
    gameItem.innerHTML = `
      <div>
        ...
        <div>
          <button id='game-${
            game.address
          }' class='primary-button'>Switch</button>
        </div>
      </div>
    `;
      ...
      if (mint.innerHTML.indexOf(mintButtonTitle) === -1) {
        ...
        setOnClick(mintButton, async () => {
          ...
          try {
            ...
            const gameData = data.effects.events.find((e) => e.moveEvent)
              .moveEvent.fields;
